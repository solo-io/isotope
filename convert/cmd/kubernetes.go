...
import (
	...
	"solo-io/isotope/convert/pkg/kubernetes"
)
...
var kubernetesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		manifests, err := kubernetes.ServiceGraphToKubernetesManifests(
			...
			serviceMaxIdleConnectionsPerHost, clientNodeSelector, clientImage, clientNamespace, environmentName, clusterName)
		exitIfError(err)
...
func init() {
	...
	kubernetesCmd.PersistentFlags().String(
		"client-namespace", "default", "the node selector for service workloads")
	kubernetesCmd.PersistentFlags().String(
